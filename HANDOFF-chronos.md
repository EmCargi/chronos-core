...
date: 2026-08-16
status: active
test_count: 632 total (17 verify_dungeon + 27 models + 52 economy + 24 besm_catalog + 28 status_effects + 36 status_ailments + 34 combat_physics + 18 combat_modifiers + 27 size_scale + 28 defence_absorption + 27 extended_actions + 34 combat_techniques + 24 bond_progression + 38 sanity_recovery + 34 defects + 27 social_combat + 32 diceless + 46 combat_maneuvers + 19 card_to_besm + 6 stage_cards + 28 guild_roster + 10 batch_ingest + 17 guild_pullover) = 631 pass, 1 skip, all runnable (2026-08-20)
...
- **Test suite at 632 tests (631 pass, 1 skip, all runnable as of 2026-08-20)** — 23 suites. Both ingest/roster suites isolate the roster DB + staging dirs to tmp_path, so live `data/` is never touched by tests.
- **Guild RPG roster still holds only 8 characters** — the pullover extractor + dry run ship the path, but the ingest is deliberately **deferred** until the design pass reformats the rest of the cast to registry-sheet format. No roster DB writes yet.
...
12. **Full Guild RPG cast pullover — extractor + dry run shipped, ingest pending** — `engine/guild_pullover.py` + `guild_pullover_dryrun.py` (2026-08-20) already parse the whole cast into `upsert_character()` payloads (both legacy + registry formats, 17 tests). **Remaining:** (a) design pass reformats the rest of the cast to the high-quality registry-sheet format (explicit stats + `Individual Action Syntax Card`) — Beril/Miri/Soren/Urkakh/Vandil are the precedent; (b) re-scan + review the updated dry run; (c) **wire in fresh with a safe-ingest guard** so the 6 hand-tuned roster rows (Eira 222 / Liora 110 / Rosivelle 181 / Sylvara 250 / Tomoe 250 / Zarlen 200 CP) are NOT clobbered by ladder-derived values — only write characters absent from the roster unless `--update-existing` is explicit; (d) resolve the 6 non-wired missing-NS files (Belne, Kaelis, Kari, Nyssa, Thora, Tilla) via the reformat pass.
