...
date: 2026-09-08
status: active
test_count: 725 total = 725 pass, 1 skip, all runnable (2026-09-08)
...
**Command palette wired 2026-09-08** — the browser now mirrors the TUI's dispatch: **30 commands** resolve with zero LLM calls (14 Phase A display + 11 Phase B deterministic math + `/startgreeting` + 4 Phase C economy writes), and `/loot` stays LLM-driven but session-scoped. Phase C writes (`/buy`, `/grant`, `/use`, `/provision`) target the **shared canonical economy DB** (CP-11 Option A — two interfaces, one game state); `economy.set_active_roster_path()` redirects for tests only. Triangulated across proposal + counter-plan rounds 1-3 (CP-1..CP-11); journal at `dev-journal/2026-09-08-chronos-core-web-commands.md`.
...
| `tests/test_browser_smoke.py` | Headless web-port tests (44: boot/title/sidebar/vitals/chat/node + Phase A display + Phase B math + Phase C economy writes + Phase D greeting fallbacks + Rich stripper + greeting-tag survival) via `streamlit.testing.v1.AppTest` — session + economy DBs redirected to tmp_path, economy restored on teardown | ~310 | 44 |
...
- **Command palette wired (2026-09-08)** — the browser now mirrors the TUI dispatch: **30 commands** resolve with zero LLM calls. Phase A display (14): `/roster` `/lore` `/settings` `/shop` `/wallet` `/inventory` `/loadout` `/greetings` `/engine` `/scv` `/techniques` `/defects` `/effects` `/location` `/threat`. Phase B deterministic math (11): `/shock` `/resist` `/fall` `/range` `/size` `/defence` `/sanity` `/recover` `/diceless` `/maneuver` `/attack` — arg parsing guarded (`try/except` → usage feed message), Rich markup stripped but semantic `[Hub]`/`[Quest]`/`[D-Rank]` preserved (CP-9). Phase C economy writes (4): `/buy` `/grant` `/use` `/provision` — **shared canonical economy DB** (CP-11 Option A, "two interfaces one game state"). `/startgreeting` commits its node then fires the Director opening (CP-4/CP-10/CP-10H). Only free text, `examine`, and `/loot` reach the LLM. 44 headless AppTest tests. (Triangulation: proposal + counter-plan rounds 1-3 CP-1..CP-11 + milestone journal, dated 2026-09-08.)
...
*Handoff updated 2026-09-08. Chronos Core v3 — the BESM 4e rules engine, now with **two interfaces on one engine**: the Rich TUI and a Streamlit web port (`browser_chronos.py`, launch with `chronos-ui`). 725 tests, local-only engine-scoped git. The web command palette went from Director-only to **30 wired commands** (14 display + 11 deterministic math + `/startgreeting` + 4 economy writes) plus session-scoped `/loot` — deterministic commands resolve in microseconds with zero LLM calls, the Rich-tag stripper preserves semantic `[Hub]`/`[Quest]` markers, and Phase C writes share the canonical economy (CP-11 Option A, "two interfaces, one game state"). The card→BESM compiler + staging bridge took the Shota roster 3 → 97 in one sweep. The LLM narrates; Python enforces. The Sixth Guard is not a suggestion — and now it's also visible, playable, and shoppable in a browser.*
